...
legacy_ids:
  - 'SRC-CORE-005'
legacy_paths:
  - 'C:\Nexus Mother Folder\00 Nexus Obsidian Vault\00 Source\07 Core Game Campaign rev0.3\SRC-CORE-005 - Ship_Phase_and_Crew_Operations.md'
...
The ship is a major game layer, but ship combat should remain much shallower than PC encounter combat unless a later accepted source pass changes that.
...
At the start of the ship phase, or when returning from a Route Node End Report, review:
...
Before an active node begins, the player should have a chance to review the available crew, select the node team, choose or adjust loadouts, and consider ship readiness.

This procedure can happen after selecting a node but must happen before the node starts if advancement, crew choice, or loadout choice would meaningfully affect play.
...
Planning and prep should have serious value. The ship phase is not only inventory cleanup; it is where route forecasting, faction pressure, crew skills, equipment, ship systems, and player plans should improve options or reduce risk.
...
Recovery, revival, injury, and consequence tradeoffs are flagged for future source work. The current Core domain establishes how they connect to the ship phase without finalizing detailed recovery math.

Current cross-domain recovery split:

- ordinary HP restoration may happen by default during ship time when the fiction supports ordinary care;
- armor may reset to functional baseline by default during ship time when ordinary repair/resupply is available;
...
Ship time should support repair, crew talk, recovery, inventory/armory updates, advancement, next route selection, and consequence processing.
...
These belong primarily to `Dashboards`, but they are preserved here because they affect the ship phase's required state visibility.
...
- Route Node End Report and ship-time display templates route to `Modes` and `Dashboards`.
