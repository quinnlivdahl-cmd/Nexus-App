...
legacy_ids:
  - 'SRC-CORE-004'
legacy_paths:
  - 'C:\Nexus Mother Folder\00 Nexus Obsidian Vault\00 Source\07 Core Game Campaign rev0.3\SRC-CORE-004 - Campaign_Loop_and_Route_Structure.md'
...
1. Create or maintain the PC and starting crew.
2. Start at the ship and review campaign state.
3. Review current goals, pressures, resources, ship state, crew state, equipment, and available route options.
4. Choose or confirm the next route node.
5. Before the node starts, complete required pre-node procedure: advancement/level-up if available, crew/team selection, loadout, ship readiness, and approach planning.
6. Start or arrive at the node.
7. Assess the node situation and choose an approach.
8. Resolve the node through a quick branching non-combat path, a mapped nonviolent/ship-stop TacMap, or a combat TacMap.
...
A node may already be selected, but DM Mode should not begin the node's active scene until required pre-node handling is complete.
...
A Route Node End Report appears after immediate encounter resolution and before free ship time, downtime, repair, crew talk, travel choice, or next-node selection.
