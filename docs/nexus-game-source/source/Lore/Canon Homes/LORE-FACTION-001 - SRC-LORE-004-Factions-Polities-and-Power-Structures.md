---
project: "Nexus"
doc_id: "LORE-FACTION-001"
legacy_ids:
  - 'SRC-LORE-004'
legacy_paths:
  - 'C:\Users\Quintin Livdahl\Nexus\Nexus\99 Archive\01 Superseded Source\00 Source Slots 2026-06-10\12 Source Lore Factions Timeline rev0.2\SRC-LORE-004 - Factions_Polities_and_Power_Structures.md'
title: "Factions_Polities_and_Power_Structures"
doc_status: "draft_active"
working_state: "domain_rebuild_full_migration"
mode_owner: "Shared"
source_role: "canon_home"
canon_status: "provisional_canon"
placement_domain: "Lore"
content_role: "canon_home"
topic_family: "factions_polities_and_power_structures"
owns_topics:
  - 'factions_polities_and_power_structures'
borrows_topics: []
created: "2026-05-14"
last_updated: "2026-06-08"
last_reviewed: "2026-06-08"
metadata_verified: true
metadata_notes: "Phase 9 normalized doc_id and placement metadata from LORE-CORE-004 to LORE-FACTION-001. Phase 10 consolidated body routing into domain-first language, repaired inherited display corruption, and preserved the active faction frame without relying on slot-era wording."
---

# Factions, Polities, and Power Structures

<!-- source-slice: lore.faction.present-day-power-map -->
## Present-day power map

The solar system is multi-polar and overloaded. No single faction controls it cleanly. Authority is distributed among treaty institutions, Earth-rooted human-continuity power, corporate sovereign habitats, upload persons and server polities, frontier communities, implant-linked conglomerations, and small crews.

<!-- source-slice: lore.faction.treaty-order-system-authority -->
## Treaty Order / System Authority

The system-level authority is not simply weak, evil, or irrelevant. It is the legal and diplomatic infrastructure many people depend on, especially in inner-system space and treaty-recognized routes.

Its failures usually come from overload:

- jurisdictional limits;
- slow evidence standards;
- sovereignty disputes;
- corporate pressure;
- emergency governance;
- personhood disagreement;
- poor reach into frontier habitats;
- rules moving slower than technology.

"UN," "Treaty Order," and "System Authority" are working labels. This branch needs final in-setting naming later.

<!-- source-slice: lore.faction.cpoh-human-continuity-power -->
## C-POH / human-continuity power

`C-POH` is a working handle, not final naming.

Its strongest moral argument is that ordinary people should not be economically, spiritually, legally, or biologically pressured into upgrading just to remain employable, valid, protected, or recognized.

C-POH is not anti-machine by default. It can plausibly tolerate repair, prosthetics, sensory restoration, pain regulation, industrial exosuit interfaces, local cybernetic control, and medical technologies that do not alter or network memory, personality, cognition, identity, or personhood.

It rejects or strongly restricts technologies that make mind, identity, memory, personality, or legal selfhood into property, software, collateral, subscription, or network surface.

C-POH is dangerous when protection becomes purity: anti-upload violence, forced de-augmentation, black-site brain scans, splicer persecution, Earth isolationism, or treating altered persons as contaminated evidence rather than people.

C-POH should not be reduced to a cartoon villain. It is a source of danger, partial truth, ugly political opportunity, and genuine fear.


<!-- source-slice: lore.faction.cpoh-sacred-boundary-refinement -->
## C-POH sacred-boundary refinement

C-POH should retain religious, metaphysical, or sacred-boundary energy. It is not only legal anti-cyberware politics and not merely anti-machine reaction. Its core wound is that commercial brain implants profaned the boundary of the person: memory, will, agency, cognition, identity, and legal selfhood became market surfaces.

Practical boundary:

- More tolerable: prosthetics, integrated tools, body-assist cyberware, non-cognitive restorative tech, local assistive devices, and medical systems that restore function without rewriting or networking the self.
- Rejected or heavily restricted: mindlink implants, mind-hacking implants, reflex/personality/cognition-invasive systems, memory/personhood networking, agency-altering implants, and technologies that make selfhood collateral, subscription, evidence package, or remote surface.

This does not make C-POH safe. Protection can become purity, and purity can become forced de-augmentation, anti-upload violence, black-site investigation, anti-splicer persecution, or sacramental politics over who counts as an unviolated person.

`Equipment` tracks the equipment implications. `Lore` tracks ideology, law, faction culture, and personhood boundary.

<!-- source-slice: lore.faction.corporate-sovereignties -->
## Corporate sovereignties

Corporate power is rooted in stability-for-ownership. Corporations gained influence through drones, logistics, AI, private security, finance, habitat construction, medical systems, identity infrastructure, and supply-chain control.

Corporate citizens may trade labor, obedience, debt, service, or contractual dependency for safety, air, food, housing, medical care, legal protection, and station access.

Corporate sovereignty is not just greed. It is also the terrifying practicality of surviving inside habitats that someone must build, repair, power, and defend.

<!-- source-slice: lore.faction.choir-collective-placeholder -->
## The Choir and the Collective placeholder

The preferred working concept is **the Choir**.

`Collective` is a placeholder, outsider shorthand, public label, misdiagnosis, or legacy term. It is not an official organized faction by default.

The best current model is:

- the Choir is the underlying mystery, pattern, phenomenon, or pressure;
- the "Collective" is the resulting odd organization of mind-implanted individuals and institutions affected by the mysterious Choir;
- affected people, executives, research teams, corporate security structures, implant communities, and linked workers may behave like a faction even if no clean faction constitution exists;
- some participants may be aware that something is happening without understanding it;
- some may be coerced, synchronized, entranced, misled, or instrumentally used;
- non-implanted people may ally with, serve, trade with, or live near Choir-affected institutions, but they do not truly join the Choir by default.

The Choir may not be something the corporate world built. It may be what happened when implant networks became large enough to hear the Signal hearing back.

<!-- source-slice: lore.faction.implant-megacorp-outbreak-seed -->
## Implant megacorp / outbreak seed

A major brain-implant or cybernetics corporation near the Jupiter moons remains the clearest early outbreak seed. The accepted seed is Secret Research Backfire: Signal research causes or reveals a disturbance that propagates through required employee brain implants.

The company may continue operating while its decisions become subtly Choir-shaped. Competing cybernetics companies may begin behaving with unusual cooperation, which should be treated as a red flag.

This is a lore-campaign engine, not a complete faction profile yet.

<!-- source-slice: lore.faction.upload-persons-and-polities -->
## Upload persons and upload polities

Upload persons and upload polities must be split.

Upload persons concern identity, continuity, copies, forks, rights, death, consent, testimony, and legal recognition.

Upload polities concern server-habitats, compute, energy dependence, cooling, defense, diplomacy, raids, deletion risks, blockades, substrate law, and strategic bargaining.

<!-- source-slice: lore.faction.frontier-independent-habitats -->
## Frontier and independent habitats

Frontier habitats, stations, mines, ships, and small settlements are brittle and practical. They are caught between corporate contracts, system law, C-POH pressure, upload bargaining, Choir incidents, pirates, smugglers, repair shortages, and survival math.

They are an ideal mission scale because small crews can still matter there.

<!-- source-slice: lore.faction.campaign-derived-lore-caution -->
## Campaign-derived lore caution

Heliomed, E43, Nereid-3, Voss, Rill, C-WARDEN, Ternary Lock, Special Review Detachment, and other campaign materials are source candidates and campaign state evidence. They should not be promoted wholesale into general lore without later review.


