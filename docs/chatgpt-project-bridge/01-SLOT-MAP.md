...
These families are planned but not populated in this first batch.
...
| Source authority summary | `20-SOURCE-AUTHORITY-SUMMARY.md` | Small map of source authority and domain-first source expectations | ChatGPT needs source orientation without full source upload |
...
3. Add current-state delta only when a specific refresh is needed.
4. Add task-specific packets only when a chat or playtest needs narrower context.
